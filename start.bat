...
@SET DBIP=10.12.191.99
@SET DBDISK=Z


@SET PYTHONIP=10.12.193.251
@SET PYTHONDISK=W

@SET DEVIP=10.12.191.112
@SET DEVDISK=Y


@SET USERNAME=skyyuan
@SET PASSWORD=skyyuan

@SET ECLIPSE_PATH=E:\eclipse\eclipse.exe
...
@CALL :Connect %DBIP% %DBDISK%
@CALL :Connect %PYTHONIP% %PYTHONDISK%
@CALL :Connect %DEVIP% %DEVDISK%

@echo begin start eclipse
@start %ECLIPSE_PATH%
@echo .
@CALL :Sleep 1

rem @ echo begin start SecureCRT
rem @start startSecureCRT.bat
rem @echo .
...
:Connect
    @echo begin connect %1 ...
    @net use %2: \\%1\%USERNAME% %PASSWORD% /user:%PASSWORD% 
    @echo connect sucess %1
    @echo .
GOTO :EOF
